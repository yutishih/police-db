...
<template>
  <div class="dashboard-home-page">
    ...
    <div
      class="row justify-content-center block-wrapper-flex content-table-margin"
    >
      <!----環保---->
      <div class="col col-split-one-third box-shadow-wrap box-shadow-wrap-left">
          ...
          <table style="width: 100%">
            <thead>
              <tr>
                <th class="col-4"></th>
                <th class="col-8">
                  <div class="col fourth-title">南科環保曲線圖</div>
                </th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="key in Object.keys(dictEnvironmentalFriendly)">
                <td class="col-4 td-style">
                  <div style="display: flex; align-items: center">
                    <label class="item-title item-rounded-box">{{
                      dictItemEn2ch[key]
                    }}</label>
                  </div>
                </td>
                  ...
                  <div style="display: flex; align-items: center">
                    <div class="col">
                      <span
                        :class="
                          getStatusClass(
                            dictEnvironmentalFriendly[key]['finalStatus']
                          )
                        "
                      ></span>
                    </div>
                    <div class="col-10">
                      <img
                        :src="
                          require('@/assets/' +
                            dictEnvironmentalFriendly[key]['ImgName'])
                        "
      ...
      <!----工安---->
      <div class="col col-split-one-third box-shadow-wrap box-shadow-wrap-mid">
        <div class="container-fluid container-padding">
            ...
            <tbody>
              <tr v-for="key in Object.keys(dictWorkerSafety)">
                ...
                <template v-if="key != 'occupationalDisaster'">
                  <td class="col-2 item-value">
                    <span
                      :class="
                        getStatusClass(
                          dictWorkerSafety[key]['M']['Array-status']
                        )
                      "
                      >{{ dictWorkerSafety[key]["M"]["Array"] }}</span
                    >
                  </td>
                  <td class="col-2 item-value">
                    <span
                      :class="
                        getStatusClass(dictWorkerSafety[key]['M']['CF-status'])
                      "
                      >{{ dictWorkerSafety[key]["M"]["CF"] }}</span
                    >
                  </td>
                  <td class="col-2 item-value">
                    <span
                      :class="
                        getStatusClass(
                          dictWorkerSafety[key]['M']['Cell-status']
                        )
                      "
                      >{{ dictWorkerSafety[key]["M"]["Cell"] }}</span
                    >
                  </td>
                  <td class="col-2 item-value">
                    <span
                      :class="
                        getStatusClass(dictWorkerSafety[key]['M']['MDL-status'])
                      "
                      >{{ dictWorkerSafety[key]["M"]["MDL"] }}</span
                    >
                  </td>
                  <td class="col-2 item-value">
                    <span
                      :class="
                        getStatusClass(dictWorkerSafety[key]['M']['FAC-status'])
                      "
                      >{{ dictWorkerSafety[key]["M"]["FAC"] }}</span
                    >
                  </td>
                </template>
                <template v-else>
                  <td class="col-2 item-value">
                    <span
                      :class="
                        getStatusClass(
                          dictWorkerSafety[key]['thisYear']['Array-status']
                        )
                      "
                      >{{ dictWorkerSafety[key]["thisYear"]["Array"] }}</span
                    >
                  </td>
                  <td class="col-2 item-value">
                    <span
                      :class="
                        getStatusClass(
                          dictWorkerSafety[key]['thisYear']['CF-status']
                        )
                      "
                      >{{ dictWorkerSafety[key]["thisYear"]["CF"] }}</span
                    >
                  </td>
                  <td class="col-2 item-value">
                    <span
                      :class="
                        getStatusClass(
                          dictWorkerSafety[key]['thisYear']['Cell-status']
                        )
                      "
                      >{{ dictWorkerSafety[key]["thisYear"]["Cell"] }}</span
                    >
                  </td>
                  <td class="col-2 item-value">
                    <span
                      :class="
                        getStatusClass(
                          dictWorkerSafety[key]['thisYear']['MDL-status']
                        )
                      "
                      >{{ dictWorkerSafety[key]["thisYear"]["MDL"] }}</span
                    >
                  </td>
                  <td class="col-2 item-value">
                    <span
                      :class="
                        getStatusClass(
                          dictWorkerSafety[key]['thisYear']['FAC-status']
                        )
                      "
                      >{{ dictWorkerSafety[key]["thisYear"]["FAC"] }}</span
                    >
                  </td>
                </template>
              </tr>
      ...
      <!----衛生---->
      <div
        class="col col-split-one-third box-shadow-wrap box-shadow-wrap-right"
      >
        <div class="container-fluid container-padding">
          ...
          <table style="width: 100%">
            ...
            <tbody>
              <tr v-for="key in Object.keys(dictHealthSafety)">
                ...
                <td class="col-2 item-value">
                  <span
                    :class="getStatusClass(dictHealthSafety[key]['M']['Array'])"
                    >{{ dictHealthSafety[key]["ComePerson"]["Array"] }}</span
                  >
                </td>
                <td class="col-2 item-value">
                  <span
                    :class="getStatusClass(dictHealthSafety[key]['M']['CF'])"
                    >{{ dictHealthSafety[key]["ComePerson"]["CF"] }}</span
                  >
                </td>
                <td class="col-2 item-value">
                  <span
                    :class="getStatusClass(dictHealthSafety[key]['M']['Cell'])"
                    >{{ dictHealthSafety[key]["ComePerson"]["Cell"] }}</span
                  >
                </td>
                <td class="col-2 item-value">
                  <span
                    :class="getStatusClass(dictHealthSafety[key]['M']['MDL'])"
                    >{{ dictHealthSafety[key]["ComePerson"]["MDL"] }}</span
                  >
                </td>
                <td class="col-2 item-value">
                  <span
                    :class="getStatusClass(dictHealthSafety[key]['M']['FAC'])"
                    >{{ dictHealthSafety[key]["ComePerson"]["FAC"] }}</span
                  >
                </td>
              </tr>
            </tbody>
